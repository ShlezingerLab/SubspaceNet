...
from src.evaluation import evaluate_hybrid_model, evaluate_model_based
...
if __name__ == "__main__":
    ...
    if commands["TRAIN_MODE"]:
        ...
        training_parameters = TrainingParams(model_type=model_type)\
            .set_batch_size(batch_size=256).set_epochs(epochs=50)\
            .set_model(system_model=samples_model, tau=tau)\
            .load_model(load_flag=True, loading_path=saving_path / simulation_filename)\
        ...
        model, loss_train_list, loss_valid_list = train(
            training_parameters = training_parameters, model_name= simulation_filename,
    ...
    if commands["EVALUATE_MODE"]:
        ...
        subspace_methods = ["esprit", "music", "r-music", "mvdr",\
        ...
        if not commands["TRAIN_MODE"]:
            evaluation_params = TrainingParams()\
                ...
                .load_model(load_flag=True, loading_path=saving_path / Path(simulation_filename))
        ...
        model_test_loss = evaluate_model(model=model, dataset=model_test_dataset, criterion=criterion,
        ...
        for algorithm in augmented_methods:
            hybrid_loss = evaluate_hybrid_model(model=model, dataset=model_test_dataset,\
